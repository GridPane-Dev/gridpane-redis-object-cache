...
Author: Jeff cleverley - forked from Till Krüss
License: GPLv3
...
class RedisObjectCache {
    ...
    public function add_admin_menu_page() {
    	...
    	$page_title = __( 'Redis Object Cache', 'gridpane-redis-object-cache');
	    ...
	    add_action( 'admin_init', array( $this, 'register_gp_redis_settings' ) );
	    add_action( 'admin_init', array( $this, 'setup_sections' ) );

    }

    public function flush_object_cache_button( $wp_admin_bar ) {

	    if ( ! current_user_can( 'manage_options' ) ) {
		    return;
	    }

	    if ( isset( $_GET['flush-object-cache-button'] )
	         && 'flush' === $_GET['flush-object-cache-button']
	         && wp_verify_nonce( $_GET['_wpnonce'], 'flush-object-cache-button' )
	    ) {
		    wp_cache_flush();
		    add_action( 'admin_notices', function () {
			    echo '<div class="notice notice-success is-dismissible"><p>Object Cache has been flushed.</p></div>';
		    } );
	    }

	    $dashboard_url = admin_url( add_query_arg( 'flush-object-cache-button', 'flush', 'index.php' ) );
	    $args = array(
		    'id'    => 'flush_object_cache_button',
		    'title' => 'Flush Object Cache',
		    'href'  => wp_nonce_url( $dashboard_url, 'flush-object-cache-button' ),
		    'meta'  => array( 'class' => 'flush-object-cache-button' )
	    );

	    $wp_admin_bar->add_node( $args );
    }

    public function register_gp_redis_settings() {
	...
	public function setup_sections( $arguments ) {
    ...
    public function enqueue_admin_styles( $hook_suffix ) {
        ...
        if ( $hook_suffix === $this->screen ) {
            ...
            wp_enqueue_style( 'gridpane-redis-object-cache', plugin_dir_url( __FILE__ ) . 'includes/admin-page.css', null, $plugin[ 'Version' ] );
        }
